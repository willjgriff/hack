...
For example, let's say you want take funds out of a DAOs wallet to cover the cost of pizza. In this case the DAO was set up so no member could withdraw funds from the wallet, instead every withdraw had be voted on, if the vote passes money is released, else the vote fails and nothing happens. Let's say you have a voting app in your DAO, and its been setup so that the permissions allow for the withdrawal of funds if a vote passes. When you make the request for funding pizza the voting app is going to ask all members if funds should be released or not. Users will vote and once all the members have voted the results are executed. Congrats, the vote passes! Because permissions have been setup the funds are transferred to your personal wallet and you can pay for that pizza.

**Setting up permissions**

Define your ACL instance for your kernel with:

```javascript{2}
acl = ACL(kernel.acl());
```

Once this has been created we now have access to running create, grand and revoke. These three functions are going to allow us to setup permissions for any application

**Create**

createPermisson() sets up permissions for an entity (app/dao/person) to execute a new role (action) for an app. This permission is executed by a manager.

```javascript
acl.createPermission(address entity, address app, bytes32 role, address manager)
```

- **entity** - The recipient of this permission.
- **app** - The app that will have this new permission.
- **role** - The action this permission can perform.
- **manager** - The admin of the app.

An example of using createPermission() would be if an admin wanted to give the a voting app the ability to remove funds from the vault app.

```javascript{2}
acl.createPermission(0x239022020, 0x239022020, TRANSFER_MONEY_ROLE, 0x239022020);
```
...
grantPermission() updates the role that was created by createPermission()

```javascript{2}
acl.grantPermission(address entity, address app, bytes32 role)
```

- **entity** - Recipient of this permission.
- **app** - App that will have this new permission.
- **role** - Action this permission can perform.

> Note:
>
> The grantPermission() action doesn’t require protection with the ACL because an entity can only make changes to a permission if that entity is the permission manager.

**Revoke**

revokePermission() removes the permission from the targeted app by revoking the role in the app from an entity.
...
> Notes:
>
> Only callable by the manager of a certain permission.

**Adding Permissions**

Apps have the choice of which actions to protect behind the ACL, as some actions may make sense to be completely public. Protecting an action behind the ACL is done in the smart contract by simply adding the authentication modifier auth() or authP()(passing the role required as a parameter) to the action. On executing the action, the auth()/authP() modifiers check with the Kernel whether the entity performing the call holds the required role or not.

**Permission managers**

When a permission is created, a Permission Manager is set for that specific permission. This manager is able to grant or revoke permission instances for that permission(app?)

To find who is the permission manager for a apps role run:

```js
acl.getPermissionManager(address app, bytes32 role)
```

To change the permission manager run:

```js
acl.setPermissionManager(address newManager, address app, bytes32 role)
```

- **newManager** - Entitiy that will take on the manager role.
- **app** - Application the permission is coming from.
- **role** - Action which is being affected.

The original manager will lose its permission instance for this role
...
Parameters are allow to perform certain computations with the arguments of a role in order to decide whether the action can be done or not. This moves the ACL for being a purely binary access list, to a more sophisticated system that allows way more granular control.

An ACL parameter is comprised of a data structure with 3 values:

- Argument Value (uint240): It is the value to compare against depending on the argument. It is a regular Ethereum memory word, that looses it 2 most significant bytes of precision. The reason for this was to allow parameters to be saved in just one storage slot, saving significant gas. Even though uint240s are used, it can be used to store any integer up to 2^30 - 1, addresses and bytes32 (in the case of comparing hashes, losing 2 bytes of precision shouldn't be a dealbreaker if the hash algorithm is secure). The only problem is when
